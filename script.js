...
function move(camera, car, model) //Moves the car(camera) and syncs the model to the car
{
        if(car !== undefined) {
                ...
                //in landscape: gamma <0 braking, gamma >0 accelerating
                        ...
                        forcev = {x: -(forcefactor)*mass*force*Math.sin(car.rotation.y), y: 0, z: -(forcefactor/6)*mass*Math.abs(Math.cos(car.rotation.y))}; 
...
function createCar() {
        ...
        var geometry = new THREE.BoxGeometry( (carbb.max.x-carbb.min.x)/2, (carbb.max.y-carbb.min.y)/2, (carbb.max.z-carbb.min.z)/2 );  //bb rotated because model also rotated
